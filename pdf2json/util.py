...
- process_image_to_json: Processes an image using OpenAI's multimodal capabilities.
- process_text_to_structured_json: Processes text to create structured JSON.
...
def extract_pages_as_images(pdf_file, tmp_images_folder, filaname_image="image"):
    ...
    """
    # pdfuim used instead of convert_from_path because it cannot handle super long pages
...
def process_text_to_structured_json(prompt, headers, model="gpt-4.1"):
    """
    Process text into structured JSON using the OpenAI API.
    
    Args:
        prompt (str): The prompt containing text to analyze.
        headers (dict): The headers to include in the API request.
        model (str, optional): The model to use. Defaults to "gpt-4.1".
